...
- 助手只允许补充表中当前为空的单元格。
- 对于已经有内容的字段，助手不会主动修改，包括卡名、基础效果、升级效果、描述、升级描述、稀有度、费用、目标、标签/关键词。
- 只有 `卡牌ID` 缺失时，助手才会根据中文卡名生成稳定英文 ID，并且只补回 `卡牌ID` 本身。
- 用户在对话里只需说明“哪些卡更新了”；助手负责去表里找到这些行并按表实现。
- 若代码与表冲突，改代码，不改表。
- 卡牌描述和升级描述只有在对应单元格为空时，才允许根据前面的效果列补全到本地化文本；如果表里已明确写好描述，则保持不动。
